...
class ProductsController extends Controller
{
			...
			foreach ($params as $result){
				...
				foreach ($newQuery as $newQuery2){
					...
					if(($reduction!=0)AND($reduction2!=0)){
						$phraseSearchResult[]=array('id'=>$newQuery2['id_product'], 'name'=>$newQuery2['name'], 'quantity'=>$newQuery2['quantity'], 'priceRed'=>($product1->countRealPrice($newQuery2['price'], $reduction)), 'result'=>$queryResult, 'priceRed2'=>($product2->countRealPrice($oldQuery2['price'], $reduction2)), 'priceResult'=>1, 'imgNumber'=>$imageNumber);
					}elseif(($reduction==0)AND($reduction2!=0)){
						$phraseSearchResult[]=array('id'=>$newQuery2['id_product'], 'name'=>$newQuery2['name'], 'quantity'=>$newQuery2['quantity'], 'price'=>number_format($newQuery2['price'], 2,'.','').'zł', 'result'=>$queryResult, 'priceRed2'=>($product2->countRealPrice($oldQuery2['price'], $reduction2)),'priceResult'=>1, 'imgNumber'=>$imageNumber);
					}elseif(($reduction!=0)AND($reduction2==0)){
						$phraseSearchResult[]=array('id'=>$newQuery2['id_product'], 'name'=>$newQuery2['name'], 'quantity'=>$newQuery2['quantity'], 'priceRed'=>($product1->countRealPrice($newQuery2['price'], $reduction)), 'result'=>$queryResult, 'price2'=>number_format($oldQuery2['price'], 2,'.','').'zł', 'priceResult'=>1, 'imgNumber'=>$imageNumber);
					}else{
						$phraseSearchResult[]=array('id'=>$newQuery2['id_product'], 'name'=>$newQuery2['name'], 'quantity'=>$newQuery2['quantity'], 'price'=>number_format($newQuery2['price'], 2,'.','').'zł', 'result'=>$queryResult, 'price2'=>number_format($oldQuery2['price'], 2,'.','').'zł', 'imgNumber'=>$imageNumber);
					}
				}
	...
	public function completeEdition(){
		...
		foreach ($selectTag as $tag){
			if($tag!=''){
				$tagNames[]=array('id'=>$tag['id_tag'],'name'=>$tag['name']);
			}else{
				$tag='';
				$tagNames='';
			}
	...
	public function updateComplete(){
		if ($_POST['text']==''){
			...
			$Query = $product1->updateDetailedBoth($_POST['id'], $_POST['newPrice'], $_POST['text'], $_POST['quantity'], $_POST['description'], $_POST['decriptionShort'], $_POST['metaTitle'], $_POST['metaDescription'], str_replace(" ","-", $link), $_POST['condition'], $_POST['active']);
		...
		if(!isset($error)){
			if (isset($_POST['howManyBases'])and $_POST['howManyBases']== 'both'){
				try{
					...
					$Query = $product2->updateDetailedBoth($_POST['id'], $_POST['newPrice'], $_POST['text'], $_POST['quantity'], $_POST['description'], $_POST['decriptionShort'], $_POST['metaTitle'], $_POST['metaDescription'], str_replace(" ","-", $link), $_POST['condition'], $_POST['active']);
