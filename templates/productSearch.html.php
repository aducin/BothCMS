...
<head>
	...
	<style type="text/css">
		...
		#text{
			margin-left:195px;width:169px; height:18px;position:absolute;
		}
		#upperInput{
		...
		#searchInput1{
			margin-left:415px;margin-right:35px;padding-top: 3px;padding-bottom: 3px;margin-top:13px;
			font-size:12pt; font-style:italic; padding-right: 11px;
		}
		#logOut{
			margin-left:433px;padding-top: 3px;padding-bottom: 3px;margin-top:13px;
...
<body>
	...
	<span>Użytkownik: <?php htmlout($finalResult['login']); ?></span>
	<?php } ?>
			...
			<form action="" method="get">
				...
				<table>		
					<tr id="idText"><td>
						...
						<label for="idnr" id="inputidtext">Wprowadź numer ID:</label>
					</td>
					<td>
						<input type="text" name="idnr" id="idnr">
					</td>
					<td>
						<input type="hidden" name="action" value="idsearch">
						<input id="searchInput" type="submit" value="Wyszukaj">
					</td>
				</tr>
			</form>
		</table>
		<hr>
		<h2>Wyświetl produkty, które spełniają następujące kryteria:</h2>
		<form action="" method="get">
			<table>
				<tr>
					<td>
						<div id="searchList">
							<label for="author">Producent produktu:</label>
						</td>
						<td>
							<select name="author" id="productAuthor">
	...
	<?php if(isset($mod)){ ?>
	<fieldset id="fldmod">
		<legend id="legendMod">Produkty podmienione</legend>
		<table>
			<tr>
				<th>Numer ID</th>
				<th>
			...
			<?php foreach ($mods as $modified): ?>
				<tr>
					<td>
						<center><?php htmlout($modified['id']); ?></center>
					</td>
					<td id="priceAndName">
						<b>
							<a href="http://ad9bis.vot.pl/tory-h0/<?php htmlout($modified['id']); ?>-cysterna-francuska-primagaz-bttb.html" target="_blank"><?php htmlout($modified['nazwa']); ?></a>
						</b>
					</td>
					<td id="priceAndName">
						<center><?php $modcena=$modified['cena']*1;
							echo $modcena.'zł';
					...
					<td>
						<?php htmlout($modified['data']); ?>
					</td>
					<td>
						<form action="?" method="get">
							<input type="hidden" name="idMod" value="<?php htmlout($modified['id']); ?>">
							<input type="submit" name="deleterow" value="Usuń wpis">
						</form>
					</td>
				</tr>
			<?php endforeach; ?>
